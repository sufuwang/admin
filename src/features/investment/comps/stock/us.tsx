...
import { USShares, USSharesSum, type TShareRowKeys, type TShareRow } from '@/data/stock'
  ...
  },
  {
    ...
    render: (r, row) => {
      if (r === 'PROFIT') {
        ...
        const ds = USShares.filter((item) => item.code === row.code).filter(
      ...
      <Table>
        ...
        <TableFooter>
          <TableRow>
            ...
            <TableCell key='costPrice'>{formatNumber(USSharesSum.cost, 'HKD')}</TableCell>
            <TableCell key='sellPrice'>-</TableCell>
            <TableCell key='serviceFee'>{formatNumber(USSharesSum.serviceFee, 'HKD')}</TableCell>
            <TableCell key='earning'>{formatNumber(USSharesSum.earning, 'HKD')}</TableCell>
            <TableCell key='earningRate'>{USSharesSum.earningRate + '%'}</TableCell>
          ...
          <TableRow>
            ...
            <TableCell key='earning'>{formatNumber(USSharesSum.earning / USShares.length, 'HKD')}</TableCell>
            <TableCell key='earningRate'>{(USSharesSum.earningRate / USShares.length).toFixed(2) + '%'}</TableCell>
            {columns.slice(-1).map((col) => (
              <TableCell key={col.key}>-</TableCell>
            ))}
          </TableRow>
